...
GROUND, OBSTACLE, CAMP = "ground", "obstacle", "camp"
...
def make_grid():
    ...
    for x, y in camps: grid[y][x] = CAMP
    return grid, obstacles, camps
...
def draw_env(grid, seen_map, drone, camps_found, camps_detected, step_count, ep, ep_reward, coverage, detected, done_flag, fuel_consumed, fuel_efficiency, title=""):
    ...
    lines = [
        ...
        f"Coverage: {int(coverage * 100)}%  Camps: {detected}  Efficiency: {fuel_efficiency:.3f}",
...
def rl_episode(Q, epsilon, grid, play_mode=False, render=False, info=""):
    ...
    while step < max_steps:
        ...
        for sx, sy in sensory_field(nx, ny, grid):
            ...
            if grid[sy][sx] == CAMP and (sx, sy) not in camps_found:
                camps_found.add((sx, sy))
                camps_detected.add((sx, sy))
                reward += 10
        ...
        if render:
            draw_env(grid, seen_map, drone, camps_found, camps_detected, step, "-", total_reward, coverage, len(camps_found), done, total_fuel_consumed, fuel_efficiency, title=info)
...
if __name__ == "__main__":
    ...
    camp_img = load_and_resize("Sprites/camp.png")
    ...
    grid, obstacles, camp_list = make_grid()
    ...
    for ep in range(1, episodes + 1):
        ...
        if ep % 10 == 0 or ep <= 5 or ep > episodes - 5:
            print(f"Ep {ep:3} | Reward={result['reward']:4} Coverage={result['coverage']*100:5.1f}% " +
                  f"Camps found={result['camps']} Fuel_Consumed={result['fuel_consumed']:.1f} Eff={result['fuel_efficiency']:.3f}")
    ...
    for i in vis_indices:
        ...
        print(f"Showing {info} | R={result['reward']} C={result['coverage']*100:.1f}% " +
              f"B={result['camps']} F={result['fuel_consumed']:.1f} E={result['fuel_efficiency']:.3f}")
